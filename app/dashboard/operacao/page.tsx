...
import { clientes, destinos, planejamento, reservas, viagens } from '@/services/bondrota';
import type { FalhaPlanejamento, Reserva, RotaDinamica, StatusPresenca, ViagemComCiclo, ViagemHorario, ViagemReserva } from '@/types/domain';

type Tab = 'reservas' | 'viagens' | 'falhas';
interface Data { bookings: Reserva[]; trips: ViagemComCiclo[]; failures: FalhaPlanejamento[]; clientNames: Map<number, string>; destinationNames: Map<number, string> }
...
export default function OperacaoPage() {
  ...
  const loader = useCallback(async () => {
    const [bookings, trips, failures, clientItems, destinationItems] = await Promise.all([reservas.list(), viagens.list(), planejamento.failures(100), clientes.list(), destinos.list()]);
    return { bookings, trips, failures, clientNames: new Map(clientItems.map((item) => [item.id, item.nome])), destinationNames: new Map(destinationItems.map((item) => [item.id, item.nome])) };
  }, []);
  const resource = useResource<Data>(loader);
...
function BookingsTable({ items, data, busy, onCancel, onDelete }: { items: Reserva[]; data: Data; busy: boolean; onCancel(id: number): void; onDelete(id: number): void }) {
