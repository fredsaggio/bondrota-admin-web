...
import { motoristas, veiculos, viagens } from '@/services/bondrota';

const LiveMap = dynamic(() => import('@/components/live-map'), { ssr: false, loading: () => <div className="app-loader !min-h-full"><span className="spinner" /><span>Carregando mapa</span></div> });
...
export default function MonitoramentoPage() {
  ...
  const liveLoader = useCallback(async () => {
    if (!selectedId) return { location: null, route: null };
    const [location, route] = await Promise.all([
      viagens.location(selectedId).catch((error) => error instanceof ApiError && error.status === 404 ? null : Promise.reject(error)),
      viagens.route(selectedId).catch((error) => error instanceof ApiError && error.status === 404 ? null : Promise.reject(error)),
    ]);
